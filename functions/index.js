  ...
  .onUpdate(async function (snapshot, context) {
    ...
    if (after.status == TRIP_STATUS_WAITING) {
      ...
      let emailData = {
        ...
        finalFare: after.fareDetails.finalFare.toFixed(2),
        tripId: key,
    ...
    if (req.body.tripId && req.body.type) {
      ...
      if (tripData) {
        ...
        let emailData = {
          ...
          companyName: companyData.name,
          currency: businessData.currency,
          finalFare: tripData.fareDetails.finalFare.toFixed(2),
...
exports.sendPushDev = functions.database.instance('wrapspeedtaxidev')
  .ref("notifications/{notification}")
  .onCreate(function (change, context) {
    const original = change.val();
    if (original.type == "riders") {
...
exports.sendPushPromoDev = functions.database.instance('wrapspeedtaxidev')
  .ref("promotions/{promotionId}")
  .onCreate(function (change, context) {
    const original = change.val();
    if (original.type == "riders") {
...
exports.sendPushNewsDev = functions.database.instance('wrapspeedtaxidev')
  .ref("news/{newsId}")
  .onCreate(function (change, context) {
    const original = change.val();
    if (original.type == "riders") {
...
exports.deleteRiderDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/passengers/{id}")
  .onDelete(async function (change, context) {
    const id = context.params.id;
    await admin.database(devMode).ref("passenger-wallets/" + id).remove();
    await admin.database(devMode).ref("passenger-push-notifications/" + id).remove();
    await admin.database(devMode).ref("passenger-corporates/" + id).remove();
    await admin.database(devMode).ref("passenger-insurances/" + id).remove();
    await admin.database(devMode).ref("passenger-admin-donations/" + id).remove();
    await admin.database(devMode).ref("passenger-addresses/" + id).remove();
    await admin.database(devMode).ref("passenger-cards/" + id).remove();
    await admin.database(devMode).ref("passenger-emergency/" + id).remove();
    await admin.database(devMode).ref("passenger-subscriptions/" + id).remove();
    await admin.auth().deleteUser(id);
  });

exports.deleteDriverDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/drivers/{id}")
  .onDelete(async function (change, context) {
    const id = context.params.id;
    await admin.database().ref("driver-wallets/" + id).remove();
...
exports.deleteAdminDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/admins/{id}")
  .onDelete(function (change, context) {
    const id = context.params.id;
    admin
...
exports.createAdminDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/admins/{id}")
  .onCreate(function (snapshot, context) {
    const key = context.params.id;
    const original = snapshot.val();
...
exports.updateAdminDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/admins/{id}")
  .onUpdate(function (snapshot, context) {
    const key = context.params.id;
    const before = snapshot.before.val();
...
exports.createDriverDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/drivers/{id}")
  .onCreate(async function (snapshot, context) {
    const key = context.params.id;
    const original = snapshot.val();
...
exports.updateDriverDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/drivers/{id}")
  .onUpdate(async function (snapshot, context) {
    const key = context.params.id;
    const before = snapshot.before.val();
...
exports.createPassengerDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/passengers/{id}")
  .onCreate(async function (snapshot, context) {
    const key = context.params.id;
    const original = snapshot.val();
...
exports.updatePassengerDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/passengers/{id}")
  .onUpdate(async function (snapshot, context) {
    const key = context.params.id;
    const before = snapshot.before.val();
...
exports.makeReportDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/trips/{tripId}")
  .onWrite(function (change, context) {
    if (!change.before.val()) {
      return false;
...
exports.tripCreateTriggerDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/trips/{tripId}")
  .onCreate(function (snapshot, context) {
    const key = context.params.tripId;
    const original = snapshot.val();
...
exports.tripUpdateTriggerDev = functions.database.instance('wrapspeedtaxidev')
  .ref("/trips/{tripId}")
  .onUpdate(async function (snapshot, context) {
    const key = context.params.tripId;
    const before = snapshot.before.val();
    ...
    admin
      ...
      const businessData = (
        await admin.database(devMode).ref("business-management").once("value")
      ).val();

      const companyData = (
        await admin.database(devMode).ref("company-details").once("value")
      ).val();
...
exports.sendSOSMessageDev = functions.https.onRequest((req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
  res.set("Access-Control-Allow-Credentials", "true"); // vital
  if (req.method === "OPTIONS") {
    // Send response to OPTIONS requests
...
exports.validateReferralCodeDev = functions.https.onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
  res.set("Access-Control-Allow-Credentials", "true"); // vital
  if (req.method === "OPTIONS") {
    // Send response to OPTIONS requests
...
exports.generateInvoiceMailDev = functions.https.onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
  res.set("Access-Control-Allow-Credentials", "true"); // vital
  if (req.method === "OPTIONS") {
    // Send response to OPTIONS requests
...
exports.scheduledFunctionDev = functions.pubsub
  .schedule("every 15 minutes")
  .onRun((context) => {
    const date = moment();
    admin
